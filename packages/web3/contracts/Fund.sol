...
contract Fund is Ownable {
  ...
  *  @dev User action by anyone
  ...
  function estimateIncome(uint _projectId) public view returns (uint) {
    uint totalVoteAmount = ballot.getTotalVotedAmount();
    uint projectVoteAmount = ballot.getVotedAmountByProjectId(_projectId);
    uint tokenBalance = baseToken.balanceOf(address(this));

    require(totalVoteAmount > 0, "Total vote amount must be greater than zero");

    return (tokenBalance * projectVoteAmount) / totalVoteAmount;
  }
  ...
  *  @dev Batch by admin
  ...
  function claim(uint _projectId) public onlyOwner {
    uint totalVotedAmount = ballot.getTotalVotedAmount();
    uint projectVotedAmount = ballot.getVotedAmountByProjectId(_projectId);
    uint tokenBalance = baseToken.balanceOf(address(this));

    require(projectVotedAmount > 0, "Insufficient project vote amount");
    require(totalVotedAmount > 0, "Total vote amount must be greater than zero");

    uint claimableToken = (tokenBalance * projectVotedAmount) / totalVotedAmount;
